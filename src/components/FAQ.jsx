    ...
    <div className="max-w-3xl mx-auto px-4 py-10 bg-white text-black">
      ...
      <div className="space-y-4">
        {faqs.map((faq, index) => (
          <div key={index}>
            <div
              className="flex justify-between items-center pb-3 border-b border-black cursor-pointer"
              onClick={() => toggleFAQ(index)}
            >
              <h3 className="text-lg font-medium">{faq.question}</h3>
              <span className="w-8 aspect-square flex items-center justify-center rounded-full bg-black text-white flex-shrink-0">
                {openIndex === index ? <FiMinus /> : <FiPlus />}
              </span>
            </div>
            {openIndex === index && (
              <p className="mt-3 text-sm text-gray-800">{faq.answer}</p>
            )}
          </div>
        ))}
      </div>
